...
export function CaseStudyEdit() {
  ...
  const [audience, setAudience] = useState<"freelance" | "job" | "both">("both");
  ...
  const loadProject = async () => {
    try {
      ...
      setAudience(target.audience || "both");
      ...
      <form onSubmit={(e) => e.preventDefault()} className="grid gap-8 lg:grid-cols-[1.4fr_0.6fr]">
        ...
        <div className="space-y-8">
          ...
          <div className="rounded-[6px] border border-white/[0.06] bg-[#0B0B0B] p-6 space-y-6">
            ...
            <div className="block">
              <span className="mb-2 block text-[9px] uppercase tracking-wider text-white/40 font-bold">Target Audience</span>
              <div className="flex gap-2">
                {(["freelance", "job", "both"] as const).map((aud) => (
                  <button
                    key={aud}
                    type="button"
                    onClick={() => setAudience(aud)}
                    className={`flex-1 py-2 text-[9px] uppercase font-bold tracking-wider rounded-[4px] border transition-all ${
                      audience === aud
                        ? "bg-white text-black border-white"
                        : "bg-[#050505] text-white/40 border-white/[0.08] hover:text-white"
                    }`}
                  >
                    {aud}
                  </button>
