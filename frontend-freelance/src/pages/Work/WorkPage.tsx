...
export function WorkPage() {
  ...
  useEffect(() => {
    const fetchLiveProjects = async () => {
      try {
        const live = await api.get("/case-studies?status=published&audience=freelance");
  ...
  const audienceFiltered = liveProjects.filter((p) => !p.audience || p.audience === "freelance" || p.audience === "both");

  // Dynamically sort projects
  const sortedProjects = [...audienceFiltered].sort((a, b) => {
    const idxA = freelanceOrder.indexOf(a.slug);
    const idxB = freelanceOrder.indexOf(b.slug);
  ...
  return (
    <div className="relative min-h-screen bg-background text-white overflow-hidden font-sans">
      <Helmet>
        <title>Portfolio Gallery — Saurabh Rathore</title>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          >
            ...
            <p className="max-w-xl text-white/60 text-base sm:text-lg leading-relaxed font-sans">
              Explore my collection of high-end interfaces, design systems, and custom React / Framer development shipped for ambitious brands.
            </p>
          </motion.div>
        </div>
      </section>
