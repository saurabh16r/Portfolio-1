...
export default function LiquidFooterBackground() {
  ...
  // Monitor visibility of Footer area. Lazy load canvas when close.
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { rootMargin: "150px" } // Start pre-loading 150px before entering viewport
    );
    observer.observe(outerRef.current);
    return () => observer.disconnect();
  }, [hasEnteredView]);

  // Load and init script progressively after viewport entry
  useEffect(() => {
    ...
    const bootUnicorn = () => {
      loadUnicornScript()
        ...
        .catch((err) => {
          console.error("Unicorn Studio script failed to load for footer:", err);
        });
    };
  ...
  }, [prefersReducedMotion, hasEnteredView]);
  ...
  return (
    ...
    >
      {/* 
        ...
        - Opacity set to 0.12 (approx. 35% of the Hero's 0.35 opacity) for a much subtler background effect
        - Lazy loaded and pauses when scrolled out of view
      */}
      {hasEnteredView && (
        <div
          ref={containerRef}
          ...
          style={{
            opacity: isLoaded && inView ? 0.12 : 0,
      ...
      {/* Subtle footer dark overlay above WebGL and below content */}
      <div
